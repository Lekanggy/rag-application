...
class Embedder:
    def __init__(self, path=None):
        # Default: look for models folder next to embedded.py (in parent folder)
        if path is None:
            # __file__ is the path to embedded.py itself
            # .parent gives the folder containing embedded.py (the parent folder)
            path = Path(__file__).parent / "models" / "Xenova" / "all-MiniLM-L6-v2"
        else:
            path = Path(path)
